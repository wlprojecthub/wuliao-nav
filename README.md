...
- 🔍 **本地模糊搜索** - 支持名称、描述、标签的多关键词匹配，英文搜索不区分大小写；同时支持按域名、完整 URL 和带路径链接定位站点，支持少量中文别名、`?q=` 搜索状态恢复和中文输入法组合输入保护
- 📱 **响应式布局** - 完美适配桌面和移动端
- ⚡ **快速加载** - 纯静态页面，无需后端
- 🎯 **智能分类** - 12 个精心分类的网站目录
...
- 🌙 **星轨 404 页面** - 提供与首页一致的 Stardust ACG 风格错误页和常用分类快捷入口；默认配置适用于站点根目录部署
...
- **JavaScript** - 交互逻辑
- **本地 Favicon** - 923 个可发布图标文件已下载到本地
- **品牌默认图标** - 对于无法可靠确认官方图标的站点，统一使用与站点品牌一致的四角星默认图标兜底；当前缺失 favicon 引用为 0
...
这些脚本分别用于本地静态预览与 404 验收、数据一致性检查、favicon 引用与资源检查。
...
脚本会计算 `style.css`、`data.js`、`default-icon.png` 和 `main.js` 的内容哈希，
更新 `main.js` 中的默认图标兜底路径，并同步更新 `index.html` 与 `404.html`
中的资源 URL。执行后再上传完整项目。

使用 Nginx 或 OpenResty 部署时，将 `deploy/nginx-cache.conf` 中的规则复制到
站点 `server` 配置块并重新加载服务器。HTML 入口需要在每次访问时重新验证，
以便浏览器发现新的资源哈希。带内容哈希的 CSS 和 JavaScript 可缓存一年；
文件名稳定的 `/assets/img/` 图片资源使用 7 天可重新验证缓存，避免默认图标和 favicon
更新后长期停留在旧版本。

`404.html` 默认使用 `/assets/...`、`/` 和 `/#category-...` 根路径，以保证根域部署下从
`/abc/test/123` 等多级错误路径进入时资源仍可正确加载。若部署到 GitHub Pages 项目路径
（例如 `/wuliao-nav/`），需要在部署版本中将 `404.html` 的资源路径、首页链接和快捷入口
统一加上 `/wuliao-nav/` 基础路径，并按托管平台配置项目级 404。不要机械替换为相对路径。
