...
- 🔒 移除已被劫持或跳转至不安全内容的失效站点条目 `magician.design` 及其遗留图标
- 🧭 修复搜索后点击一级分类或二级分类仍停留在筛选结果的问题
- 🖼️ 将 `assets/img/default-icon.png` 修复为真实 PNG 文件，并保留 `cloud.hosthatch.com` 默认图标兜底
- ✦ 将默认图标替换为与站点品牌一致的四角星图标，并增加默认图标缓存刷新机制
- ⚡ 拆分静态资源缓存策略：CSS 和 JavaScript 保持一年 immutable，图片改为 7 天可重新验证缓存
- ♿ 为移动端菜单补充 `aria-controls` 和动态 `aria-expanded` 状态，并在减少动态效果模式下关闭脚本平滑滚动
- 🛡️ 补齐分类动态文本和链接属性转义，并限制动态 DOM 标识符格式
- 🧹 修复 `donationcoder` 站点名称的前导空格数据格式错误
- 🖼️ 将 28 个无法正常解码的 SVG 伪 PNG 图标定向转换为真实 PNG
- 📊 当前共收录 `995` 个网站，其中 AI 工具分类为 `157` 个
- 📝 明确 `404.html` 默认适用于站点根目录部署；子目录部署需要统一调整基础路径配置
